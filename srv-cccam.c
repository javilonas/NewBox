...
static int32_t cc_sendinfo_cli(struct cc_client_data *cli, int32_t sendversion)
{
...
int32_t cc_sendcard_cli(struct cardserver_data *cs, struct cc_client_data *cli, int32_t uphops)
{
	int32_t j;
...
void *cc_connect_cli(struct struct_clicon *param)
{
	...
	if ((i = recv_nonb(sock, buf, 6, 3000)) == 6) {
		...
		if (memcmp(buf+1, "Ccam\0", 5) != 0) {
			debugf(" CCcam: account '%s' wrong password!\n", usr);
			close(sock);
			return -1;
		}
	} else
...
void cc_store_ecmclient(int32_t ecmid, struct cc_client_data *cli)
{

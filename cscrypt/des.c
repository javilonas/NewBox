...
int32_t des_encrypt(byte *buffer, int32_t len, byte *deskey)
{
  byte checksum = 0;
  byte noPadBytes;
  byte padBytes[7];
...
int32_t des_decrypt(byte *buffer, int32_t len, byte *deskey)
{
  char ivec[8];
  char nextIvec[8];
  int32_t i;
...
void des_login_key_get(byte *key1, byte *key2, int32_t len, byte *des16)
{
  byte des14[14];
  int32_t i;
